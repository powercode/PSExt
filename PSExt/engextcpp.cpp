...
ExtCommandDesc::ClearArgs(void)
{
	...
	m_CustomArgDescLong = nullptr;
	m_CustomArgDescShort = nullptr;
	m_OptionChars = "/-";
	m_ArgStrings = nullptr;
	m_NumArgs = 0;
	m_NumUnnamedArgs = 0;
	m_Args = nullptr;
}
...
ExtCommandDesc::ParseArgDesc(void)
{
	...
	while (*Scan)
	{
		...
		case 'e':
			...
			Arg->ExpressionEvaluator = nullptr;
...
ExtCommandDesc::FindArg(_In_ PCSTR Name) const
{
...
ExtCommandDesc::ArgDesc* WINAPI
ExtCommandDesc::FindUnnamedArg(_In_ ULONG Index) const
{
	ArgDesc* Check = m_Args;
	for (ULONG i = 0; i < m_NumArgs; i++, Check++)
	{
	...
	_Out_ PULONG LongestName)
{
	*Commands = s_Commands;
	s_Commands = nullptr;
	*LongestName = static_cast<ULONG>(ExtCommandDesc::s_LongestCommandName);
...
ExtExtension::ExtExtension(void)
	...
	m_KnownStructs(nullptr),
	m_ProvidedValues(nullptr),
	...
	m_OutputWidth(0), 
	m_ActualMachine(IMAGE_FILE_MACHINE_UNKNOWN), 
	m_Machine(IMAGE_FILE_MACHINE_UNKNOWN), 
	m_PageSize(0), 
	m_PtrSize(0), 
	m_NumProcessors(0), 
	m_OffsetMask(0),
	m_DebuggeeClass(DEBUG_CLASS_UNINITIALIZED),
	...
	m_LeftIndent(0),
	m_AllowWrap(true),
	m_TestWrap(0), m_TestWrapChars(0),
	m_AppendBuffer(nullptr),
	m_AppendBufferChars(0),
	m_AppendAt(nullptr), 
	m_Commands(nullptr), m_LongestCommandName(0), m_CallStatus(0), m_MacroStatus(0),
	m_DbgHelp(nullptr),
	m_SymMatchStringA(nullptr),
	...
	_Out_ PULONG Flags)
{
	...
	ExtCommandDesc::Transfer(&m_Commands, &m_LongestCommandName);
	...
	*Version = DEBUG_EXTENSION_VERSION(m_ExtMajorVersion, m_ExtMinorVersion);
...
void WINAPIV
ExtExtension::Err(_In_ PCSTR Format, ...) const
{
	va_list Args;
...
void WINAPIV
ExtExtension::Verb(_In_ PCSTR Format, ...) const
{
	va_list Args;
...
void WINAPIV
ExtExtension::Out(_In_ PCWSTR Format, ...) const
{
	va_list Args;
...
void WINAPIV
ExtExtension::Warn(_In_ PCWSTR Format, ...) const
{
	va_list Args;
...
void WINAPIV
ExtExtension::Err(_In_ PCWSTR Format, ...) const
{
	va_list Args;
...
void WINAPIV
ExtExtension::Verb(_In_ PCWSTR Format, ...) const
{
	va_list Args;
...
void WINAPIV
ExtExtension::Dml(_In_ PCSTR Format, ...) const
{
	va_list Args;

	va_start(Args, Format);
	m_Control->ControlledOutputVaList(DEBUG_OUTCTL_AMBIENT_DML, m_OutMask, Format, Args);
	va_end(Args);
}

void WINAPIV
ExtExtension::DmlWarn(_In_ PCSTR Format, ...) const
{
	va_list Args;

	va_start(Args, Format);
	m_Control->ControlledOutputVaList(DEBUG_OUTCTL_AMBIENT_DML, DEBUG_OUTPUT_WARNING, Format, Args);
	va_end(Args);
}

void WINAPIV
ExtExtension::DmlErr(_In_ PCSTR Format, ...) const
{
	va_list Args;

	va_start(Args, Format);
	m_Control->ControlledOutputVaList(DEBUG_OUTCTL_AMBIENT_DML, DEBUG_OUTPUT_ERROR, Format, Args);
	va_end(Args);
}

void WINAPIV
ExtExtension::DmlVerb(_In_ PCSTR Format, ...) const
{
	va_list Args;

	va_start(Args, Format);
	m_Control->ControlledOutputVaList(DEBUG_OUTCTL_AMBIENT_DML, DEBUG_OUTPUT_VERBOSE, Format, Args);
	va_end(Args);
}

void WINAPIV
ExtExtension::Dml(_In_ PCWSTR Format, ...) const
{
	va_list Args;

	va_start(Args, Format);
	m_Control4->ControlledOutputVaListWide(DEBUG_OUTCTL_AMBIENT_DML, m_OutMask, Format, Args);
	va_end(Args);
}

void WINAPIV
ExtExtension::DmlWarn(_In_ PCWSTR Format, ...) const
{
	va_list Args;

	va_start(Args, Format);
	m_Control4->ControlledOutputVaListWide(DEBUG_OUTCTL_AMBIENT_DML, DEBUG_OUTPUT_WARNING, Format, Args);
	va_end(Args);
}

void WINAPIV
ExtExtension::DmlErr(_In_ PCWSTR Format, ...) const
{
	va_list Args;

	va_start(Args, Format);
	m_Control4->ControlledOutputVaListWide(DEBUG_OUTCTL_AMBIENT_DML, DEBUG_OUTPUT_ERROR, Format, Args);
	va_end(Args);
}

void WINAPIV
ExtExtension::DmlVerb(_In_ PCWSTR Format, ...) const
{
	va_list Args;

	va_start(Args, Format);
	m_Control4->ControlledOutputVaListWide(DEBUG_OUTCTL_AMBIENT_DML,  DEBUG_OUTPUT_VERBOSE, Format, Args);
	va_end(Args);
}
...
void WINAPIV
ExtExtension::OutWrapVa(_In_ PCSTR Format, _In_ va_list Args)
{
...
PSTR WINAPI
ExtExtension::RequestCircleString(_In_ ULONG Chars) const
{
	...
	if (static_cast<ULONG_PTR>(s_CircleString - s_CircleStringBuffer) >
...
PSTR WINAPI
ExtExtension::CopyCircleString(_In_ PCSTR Str) const
{
	PSTR Buf;
	ULONG Chars;
...
PSTR WINAPI
ExtExtension::PrintCircleStringVa(_In_ PCSTR Format, _In_ va_list Args) const
{
	StringCbVPrintf(s_String, sizeof(s_String), Format, Args);
...
PSTR WINAPIV
ExtExtension::PrintCircleString(_In_ PCSTR Format,	...) const
{
	PSTR Str;
	va_list Args;
...
void WINAPI
ExtExtension::SetAppendBuffer(_In_reads_(BufferChars) PSTR Buffer, 	_In_ ULONG BufferChars)
{
...
ExtExtension::AppendBufferString(_In_ PCSTR Str)
{
	...
	if (Chars > m_AppendBufferChars ||
		static_cast<ULONG_PTR>(m_AppendAt - m_AppendBuffer) > m_AppendBufferChars - Chars)
	{
...
void WINAPI
ExtExtension::AppendStringVa(_In_ PCSTR Format,	_In_ va_list Args)
{
	...
	_Out_ PULONG64 ModBase) const
{
	...
	if ((Status = m_Advanced2->Request(DEBUG_REQUEST_GET_CACHED_SYMBOL_INFO, Cookie, sizeof(*Cookie), &Info, sizeof(Info), nullptr)) == S_OK)
	{
	...
	if ((Status = m_Symbols->GetSymbolTypeId(Symbol,  &Info.Id, &Info.ModBase)) != S_OK)
	{
		ThrowStatus(Status, "Unable to get type ID of '%s'", Symbol);
	}
	...
	*ModBase = Info.ModBase;
	...
	if (m_Advanced2->Request(DEBUG_REQUEST_ADD_CACHED_SYMBOL_INFO, &Info, sizeof(Info), Cookie, sizeof(*Cookie), nullptr) != S_OK)
	{
		*Cookie = 0;
	}
...
ExtExtension::GetCachedSymbolInfo(_In_ ULONG64 Cookie,
	_Out_ PDEBUG_CACHED_SYMBOL_INFO Info) const
{
	HRESULT Status;
...
ExtExtension::FindSymMatchStringA(void)
{
	...
	m_SymMatchStringA = PFN_SymMatchStringA(GetProcAddress(m_DbgHelp, "SymMatchStringA"));
...
bool WINAPI
ExtExtension::ModuleHasGlobalSymbols(_In_ ULONG64 ModBase) const
{
...
bool WINAPI
ExtExtension::ModuleHasTypeInfo(_In_ ULONG64 ModBase) const
{
	IMAGEHLP_MODULEW64 Info;
	...
	_In_ ULONG TimeoutMilliseconds) const
{
	...
	GetExprU64("@$callret", MAXULONG64, &RetVal);
	return RetVal;
}
...
PCSTR WINAPI
ExtExtension::GetUnnamedArgStr(_In_ ULONG Index) const
{
...
ULONG64 WINAPI
ExtExtension::GetUnnamedArgU64(_In_ ULONG Index) const
{
	if (Index >= m_NumUnnamedArgs)
	{
		ThrowInvalidArg("Invalid unnamed argument index %u, only given %u",
...
void WINAPIV
ExtExtension::ThrowRemote(_In_ HRESULT Status,	_In_ PCSTR Format,	...) const
{
...
void WINAPIV
ExtExtension::ThrowStatus(_In_ HRESULT Status,	_In_ PCSTR Format,	...) const
{
	ExtStatusException Ex(Status);
	va_list Args;
...
ExtExtension::QueryMachineInfo(void)
{
	...
	if ((Status = m_Control->GetEffectiveProcessorType(&m_Machine)) != S_OK ||
		(Status = m_Control->GetPageSize(&m_PageSize)) != S_OK ||
		// IsPointer64Bit check must be last as Status
		// is used to compute the pointer size below.
		FAILED(Status = m_Control->IsPointer64Bit()))
	{
		return Status;
	}
	if (Status == S_OK)
...
ExtExtension::Query(_In_ PDEBUG_CLIENT Start)
{
	...
	if ((Status = m_Control->GetDebuggeeType(&m_DebuggeeClass, &m_DebuggeeQual)) != S_OK ||
		(Status = m_Client->GetOutputWidth(&m_OutputWidth)) != S_OK ||
		(Status = m_Control->GetActualProcessorType(&m_ActualMachine)) != S_OK ||
	...
	_In_opt_ PCSTR RawName)
{
	...
	catch (ExtException Ex)
	{
			...
			Ex.GetMessage())
		{
			...
			FAILED(Ex.GetStatus()))
		{
			m_Control->Output(DEBUG_OUTPUT_ERROR, "ERROR: %s%s: extension exception 0x%08x.\n", PreName, Name, Ex.GetStatus());
		}
	...
	_Inout_opt_ PULONG BufferChars)
{
		...
		BufferChars > nullptr)
	{
		...
		while (Struct && Struct->TypeName)
		{
			...
			{
				Status = CallKnownStruct(Client, Struct, Flags, Offset, Buffer, BufferChars);
				break;
			}
	...
	_In_opt_ PCSTR Args)
{
	...
	for (;;)
	{
		...
		if (ImplicitNamedArg ||
			strchr(Desc->m_OptionChars, *Scan) != nullptr)
		{
			//
		...
		Scan = PSTR(SetRawArgVal(Check, nullptr, false, Scan, true, 0));
...
ULONG64 WINAPI
ExtRemoteData::GetData(_In_ ULONG Request) const
{
	g_Ext->ThrowInterrupt();
...
ExtRemoteData::GetString(_Out_writes_opt_(BufferChars) PSTR Buffer,
	...
	_In_ ULONG MaxChars,
	_In_ bool MustFit,
	_Out_opt_ PULONG NeedChars) const
{
	HRESULT Status;
...
ExtRemoteTyped::Set(_In_ PCSTR Expr)
{
	...
	PSTR Msg = g_Ext->PrintCircleString("Set: unable to evaluate '%s'", Expr);
	...
	_In_ ULONG64 Offset)
{
	...
	PSTR Msg = g_Ext->PrintCircleString("Set: unable to evaluate '%s' for 0x%I64x",	Expr, Offset);
	...
	_In_ ULONG64 Offset)
{
	...
	if (FAILED(Status))
	{
		g_Ext->ThrowRemote(Status, "ExtRemoteTyped::Set from type and offset");
	}
	...
	_Out_opt_ PULONG Out32)
{
	...
	ExtData = reinterpret_cast<EXT_TYPED_DATA*>(DataHolder.Get(ExtDataBytes));
	ExtraData = reinterpret_cast<PBYTE>(ExtData + 1);
...
ULONG64 WINAPI
ExtNtOsInformation::GetNtDebuggerData(_In_ ULONG DataOffset, _In_ PCSTR Symbol,	_In_ ULONG Flags)
{
	ULONG64 Data;
	...
	if (g_Ext->m_Data->ReadDebuggerData(DataOffset, &Data, sizeof(Data), nullptr) == S_OK)
	{
		return Data;
	}
	...
	if (g_Ext->m_Symbols->GetOffsetByName(Symbol, &Data) != S_OK)
	{
		g_Ext->ThrowRemote(E_INVALIDARG,  "Unable to find '%s', check your NT kernel symbols", Symbol);
	}
...
ExtDefine* WINAPI
ExtDefineMap::Map(_In_ ULONG64 Value) const
{
...
ExtDefineMap::MapStr(_In_ ULONG64 Value,
	_In_opt_ PCSTR InvalidStr) const
{
...
DllMain(HANDLE Instance, ULONG Reason, PVOID Reserved)
{
	...
	case DLL_PROCESS_ATTACH:
		ExtExtension::s_Module = static_cast<HMODULE>(Instance);
		break;
	}
...
EXTERN_C void CALLBACK
DebugExtensionNotify(_In_ ULONG Notify,	_In_ ULONG64 Argument)
{
	if (!g_Ext.IsSet())
	{
		return;
	...
	_Inout_opt_ PULONG BufferChars)
{
	...
	return g_Ext->HandleKnownStruct(Client, Flags, Offset, TypeName, Buffer, BufferChars);
}
	...
	_Out_ PULONG BufferNeeded)
{
	...
	return g_Ext->HandleQueryValueNames(Client, Flags, Buffer, BufferChars, BufferNeeded);
}
