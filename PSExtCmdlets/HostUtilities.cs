...
namespace PSExt
{
	...
	public class HostUtilities
	{
		...
		internal static PSObject GetDollarProfile(string allUsersAllHosts, string allUsersCurrentHost,
			string currentUserAllHosts, string currentUserCurrentHost)
		{
		...
		internal static PSCommand[] GetProfileCommands(string shellId, bool useTestProfile)
		{
			...
			var allUsersAllHosts = GetFullProfileFileName(null, false, useTestProfile);
			var allUsersCurrentHost = GetFullProfileFileName(shellId, false, useTestProfile);
			var currentUserAllHosts = GetFullProfileFileName(null, true, useTestProfile);
			var currentUserCurrentHost = GetFullProfileFileName(shellId, true, useTestProfile);
			var dollarProfile = GetDollarProfile(allUsersAllHosts, allUsersCurrentHost, currentUserAllHosts,
		...
		internal static string GetFullProfileFileName(string shellId, bool forCurrentUser, bool useTestProfile)
		{
			...
			var profileName = useTestProfile ? "profile_test.ps1" : "profile.ps1";
			...
			var fullPath = Path.Combine(basePath, profileName);
		...
		{
			const string engineKeyPath = PowerShellRootKeyPath + "\\" +
			                             RegistryVersionKey + "\\" + PowerShellEngineKey;

			using (var engineKey = Registry.LocalMachine.OpenSubKey(engineKeyPath))
			{
				if (engineKey != null)
				{
